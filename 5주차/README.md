...
- Training from scratch
	- 구조만 가져오고 학습된 가중치는 가져오지 않는다.
- Full Fine Tuning (No Freeze)
	- 구조와 학습된 가중치를 갖고 온 후 그 가중치를 시작점으로 삼아 가중치 업데이트
  	- 데이터가 충분하고 도메인이 많이 다를 때 적합하다.
- Partial Fine Tuning
	- 구조와 학습된 가중치를 갖고 온 후 가중치 일부를 Freeze 시켜놓고 나머지 가중치를 학습 시킨다.
  	- 완전 동결 후 헤드만 학습 (전형적인 전이 학습)
  		- 특성 추출기(backbone)는 고정하고 분류 헤드만 교체해서 학습한다.
  	  	- 데이터가 적거나 도메인이 비슷할 때 적합하다.
  	- 부분 동결 (레이어별 선택)
  		- 앞쪽 레이어(저수준 특성)는 고정하고 뒤쪽 레이어만 Fine tuning
  	 	- 중간 정도의 데이터와 도메인 차이가 있을 때 적합하다.
