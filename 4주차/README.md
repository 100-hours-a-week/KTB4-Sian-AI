...
<img width="295" height="194" alt="Image" src="https://github.com/user-attachments/assets/a0740b53-a022-4cae-abb9-15232c809a52" />

-> epoch수를 늘렸음에도 오히려 정확도가 떨어졌다. Loss를 살펴보면 손실값이 줄어들다가 후반에는 오히려 다시 증가하는 추세를 보임.

-> 무작정 epoch 수를 늘리는 것은 성능 증가에 도움이 되지않음. 

-> 또한 learning rate 등을 함께 고려해야함.


## 위클리 챌린지 회고
### 고민한 점
- 이미지 데이터를 불러와서 학습에 사용하는 순간부터 학습 속도가 매우 떨어져서 GPU 사용 설정이 필요해 강의 교재를 참고하여 코랩에서 GPU 사용하는 방법을 적용시켰다.
- 인터넷 환경에 따라? CIFAR-10 데이터셋 다운로드 속도가 너무 많이 차이난다.
	- 평소에는 10분 걸렸는데 다른 때에 해보니 2초밖에 걸리지 않음.
- 7번 문제에서 epoch 수 10번이 적은 것 같아서 epoch 수를 100번으로 늘려서 실험해보았으나 성능 개선에 도움이 되지 않았다. 이는 단순히 학습을 많이 시키는 것이 성능 향상에 도움이 되는 것이 아니라 여러가지(모델 구조, 옵티마이저, 학습률 등)를 함께 고려해야 성능을 향상시킬 수 있음을 의미한다.

### 개선 방안
- CNN 모델 구조를 개선하거나 에포크 수, 모델 구조, Loss, Optimizer 등을 바꿔가며 성능을 비교해보기.
	- 무작정 바꾸지 말고 어떤 식으로 영향이 갈지를 고려하기.
- 코랩 환경에서는 한계점이 많음 -> 공부를 위한 환경
- 아나콘다 환경에 익숙해져 있어서 다른 환경에서 학습 시키는 것에 대해 공부해야 한다.
